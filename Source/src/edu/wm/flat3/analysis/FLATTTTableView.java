...
import edu.wm.flat3.analysis.impact.actions.*;
...
public class FLATTTTableView extends ViewPart  implements IShowInTarget,IShowInSource  {
	...
	private void createTableSorter() {
		...
		sorter = new FLATTTTableViewSorter(
				...
				new TableColumn[] {rankColumn, iconColumn, nameColumn, probabilityColumn, markColumn, fullNameColumn, featureColumn},
				new ViewerSorter[] {
						comp.getRankComparator(),
	...
	//	EIG = JRipplesEIG.getEIG();
		...
		table.setSortColumn(rankColumn);
	...
	private void createColumns(Table table) {
		rankColumn = new TableColumn(table, SWT.LEFT);
	...
	private void fillLocalToolBar(IToolBarManager pManager)
	{
		...
		showTrace = new ShowTraceAction();
		showLucene = new ShowLuceneAction();
		showMSR = new ShowMSRAction();
		
		pManager.add(showLucene);
		pManager.add(showTrace);
		pManager.add(showMSR);
		...
		pManager.add(new Separator());
		exportTraceAction = new ExportTraceAction(this);
		importTraceAction = new ImportTraceAction(this);
		reRunTraceAction = new StartTraceAction();
		reRunTraceAction.setEnabled(FLATTT.traceRun);
		pManager.add(reRunTraceAction);
		pManager.add(exportTraceAction);
		pManager.add(importTraceAction);
		
		pManager.add(new Separator());
		exportHistoryAction = new ExportMiningAction(this);
		importHistoryAction = new ImportMiningAction(this);
		reChooseVSEAction = new ReChooseVSEAction(this);
		pManager.add(reChooseVSEAction);
		pManager.add(exportHistoryAction);
		pManager.add(importHistoryAction);
	...
	public void updateToolbarButtons() {
		if (combinationalAction != null) { // if buttons have been created already
			if (FLATTT.luceneRun && FLATTT.luceneShown)
				showLucene.setImageDescriptor(AbstractUIPlugin.imageDescriptorFromPlugin(
						FLATTT.ID_PLUGIN, "icons/luceneon.png"));
			else
				showLucene.setImageDescriptor(AbstractUIPlugin.imageDescriptorFromPlugin(
						FLATTT.ID_PLUGIN, "icons/luceneoff.png"));
			if (FLATTT.traceRun && FLATTT.traceShown)
				showTrace.setImageDescriptor(AbstractUIPlugin.imageDescriptorFromPlugin(
						FLATTT.ID_PLUGIN, "icons/traceon.png"));
			else
				showTrace.setImageDescriptor(AbstractUIPlugin.imageDescriptorFromPlugin(
						FLATTT.ID_PLUGIN, "icons/traceoff.png"));
			if (FLATTT.MSRRun && FLATTT.MSRShown)
				showMSR.setImageDescriptor(AbstractUIPlugin.imageDescriptorFromPlugin(
						FLATTT.ID_PLUGIN, "icons/msron.png"));
			else
				showMSR.setImageDescriptor(AbstractUIPlugin.imageDescriptorFromPlugin(
						FLATTT.ID_PLUGIN, "icons/msroff.png"));
			showTrace.setEnabled(FLATTT.traceRun);
			showLucene.setEnabled(FLATTT.luceneRun);
			showMSR.setEnabled(FLATTT.MSRRun);
			
			reChooseVSEAction.setEnabled(FLATTT.MSRRun);
			exportHistoryAction.setEnabled(FLATTT.MSRRun);
			//importHistoryAction should always be enabled.
			...
			reRunTraceAction.setEnabled(FLATTT.traceRun);
			exportTraceAction.setEnabled(FLATTT.traceRun);	
		}
	}
	...
	private void fillContextMenu(IMenuManager pManager)
	{
		...
		SetSeedMethodAction setSeed = new SetSeedMethodAction();
		setSeed.setEnabled(getSelectedItems().size()==1 &&
				getSelectedItems().get(0).getNodeIMember() instanceof IMethod);
		pManager.add(setSeed);
	}
